...
class ReasignarModal extends React.Component {
  async componentDidMount() {
    ...
    try {
      promises.push(dispatch(ejecutivosCreators.fetchListEjecutivo()));
      promises.push(dispatch(detalleSiniestroCreators.fetchDataSinister(numSiniestro)));
  ...
  cancelarModalReaperturar = () => {
    ...
    dispatch(detalleSiniestroCreators.fetchDataSinister(numSiniestro));

    resetFields('ejecutivoReasignar');
  ...
  aceptarModalReaperturar = async () => {
    ...
    const personaActual = parseInt(dataSinister.idEjecutivo);
            ...
            <Row gutter={24}>
              <Col xs={24} sm={42} md={24} lg={24} xl={24}>
                ...
                <div className="claims-rrgg-description-list-index-detail">
                  <span>{ejecutivoActual === 'UNDEFINED' ? '...' : ejecutivoActual}</span>
                </div>
              </Col>
              <Col xs={24} sm={42} md={24} lg={24} xl={24}>
                <Form.Item
